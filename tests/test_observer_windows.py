...
class ObserverWindowsSourceTests(unittest.TestCase):
    ...
    def test_launcher_uses_the_owned_webview_host_without_edge_identity(self) -> None:
        ...
        self.assertIn("ActivateProcessWindow", source)
        self.assertIn("ShowWindowAsync", source)
        self.assertIn("SetForegroundWindow", source)
        self.assertIn("ActivateProcessWindow(", source)
        self.assertIn("$existing.process.Id", source)
    ...
    def test_native_host_owns_its_window_icon_and_read_only_webview(self) -> None:
        ...
        self.assertIn("WindowState = FormWindowState.Maximized", program)
        self.assertIn("TryActivateExistingManagedWindow", program)
        self.assertIn("TryActivateWindow(Handle)", program)
        self.assertIn("ShowWindowAsync", identity)
        self.assertIn("SetForegroundWindow", identity)
        self.assertNotIn("观察台页面加载失败。", program)
        self.assertIn("CreateNoWindow = true", program)
        self.assertIn('"pythonw.exe"', program)
        self.assertIn('ArgumentList.Add("-m")', program)
        self.assertIn('ArgumentList.Add("llm_backend_toolkit")', program)
