...
class ContractFileTests(unittest.TestCase):
    ...
    def test_current_model_docs_keep_35b_explicit_and_qwen38_as_default(self):
        ...
        self.assertIn("`qwen3.6-35b:256k`", crosscheck)
        self.assertIn("`unverified/configured`", crosscheck)
        self.assertIn("`live_verified=false`", crosscheck)
        self.assertIn("`attempted_failed`", crosscheck)
        self.assertIn("`2ecc37e425fc4e31ab476ea5d88b5521`", crosscheck)
        self.assertIn("`aicli.recovery.capture_exception`", crosscheck)
        self.assertIn("这次历史尝试不是 PASS", crosscheck)
        self.assertIn("本次失败后没有再次执行 Live", crosscheck)
