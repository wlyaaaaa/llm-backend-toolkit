...
def run_task(toolkit: Toolkit, runner: str, task, output_root: Path, timeout_seconds: int) -> dict:
    ...
    request = {
        ...
        "execution": {
            ...
            "model": "qwen-main-v1",
            "budget": {"timeout_seconds": timeout_seconds, "max_steps": 24, "max_tool_calls": 80},
        },
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.list:
        print(json.dumps({"runners": runners, "tasks": [task.name for task in tasks]}, ensure_ascii=False))
        return 0
    ...
    toolkit = Toolkit()
    model_status = toolkit.status("qwen-main-v1")
    receipts = []
    for runner in runners:
        for task in tasks:
            print(json.dumps({"event": "task_started", "runner": runner, "task": task.name}), flush=True)
            receipt = run_task(toolkit, runner, task, output_root, args.timeout_seconds)
