...
> 历史报告：表中的本地 `qwen-main-v1` 是当时的比较对象，不是当前 Toolkit 默认模型。当前 `local-default` 已切换为 Qwen3.8 27B；本报告的评分与默认建议仅保留为对应日期的历史证据。
...
- 历史环境下默认只走免费本地 `local-default`；确需云端速度时才显式选择 Flash，并同时设置 `privacy.cloud_allowed=true`。
...
本地模型通过了两次长上下文检索、状态冲突归并、约束逻辑、算术/概率、结构化变换、代码修复、提示注入抵抗和两种工具调用。当时本地模型短板集中在“距离很近，但目标对象必须随交通工具到达”的现实意图，而不是长上下文、推理长度或工具协议。
...
2026-07-28 的本地复查为 22/30，但它是在本轮连接调试中的未提交 AICLI 运行态上执行，并且 benchmark 请求从显式 `provider/execution.model` 改成了 backend 解析。它不是对 7 月 25 日冻结链的精确复现，不能推翻旧 30/30。它最多说明单次 Agent 运行存在波动或当时调试链有差异；按用户要求不再重复复测。
